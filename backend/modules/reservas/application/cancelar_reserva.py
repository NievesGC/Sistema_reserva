"""
Caso de uso: Cancelar una reserva confirmada.

Transición de estado: confirmada → cancelada.
...
_TRANSICION_ORIGEN = EstadoReserva.CONFIRMADA
_TRANSICION_DESTINO = EstadoReserva.CANCELADA


class CancelarReservaUseCase:
    """
    Cancela una reserva que está en estado confirmada.
    ...
    async def execute(self, reserva_id: str) -> Result:
        ...
        Args:
            reserva_id: UUID de la reserva a cancelar.

        Returns:
            Ok(Reserva) con la reserva cancelada.
        ...
        reserva_cancelada = resultado_update.value
        ...
        try:
            ...
            logger.error(
                "cancelar_reserva_error_liberar_plazas",
                reserva_id=reserva_id,
                error_type=type(exc).__name__,
            )

        # Notificar al cliente (sin interrumpir si falla)
        reserva_dict = _reserva_a_dict(reserva_cancelada)
        notif_uc = EnviarNotificacionCancelacion(servicio=self._notificaciones)
        await notif_uc.execute(reserva_dict)

        logger.info("cancelar_reserva_ok", reserva_id=reserva_id)
        return Ok(reserva_cancelada)
